...
RUN --mount=type=cache,id=nuget,target=/root/.nuget/packages \
    dotnet publish "/source/src/Alexandria.FileApi/Alexandria.FileApi.csproj" -a ${TARGETARCH/amd64/x64} --use-current-runtime --self-contained false -o /app
...
ENTRYPOINT ["dotnet", "Alexandria.FileApi.dll"]